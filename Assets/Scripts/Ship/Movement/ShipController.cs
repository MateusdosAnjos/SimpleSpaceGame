...
public class ShipController : Status
{
    ...
    public GameObject specialCooldownBar;
    ...
    [Header("My Private")]
    private Slider specialCooldownSlider;
    private Slider manaSlider;
    private Slider slowMotionSlider;
    ...
    void Start()
    {
        ...
        slowMotionSlider.maxValue = slowMotionMaxTime;
        slowMotionSlider.value = slowMotionMaxTime;
        manaSlider.value = currentManaToSpecial;
    }
    ...
    void Update()
    {
        if (!GameManager.Instance.endGame)
        {
            ...
            else
            {
                ...
                specialCooldownSlider.value = currentSpecialRechargTime/specialRechargTime;
                ...
                if (Input.GetButton("SpecialWeapon1"))
                {
                    currentSpecialAble = SpecialType.BOMB;
                }
                else if (Input.GetButton("SpecialWeapon2"))
                {
                    currentSpecialAble = SpecialType.LAZER;
                }
                else if (Input.GetButton("SpecialWeapon3"))
                {
                    currentSpecialAble = SpecialType.FIRE;
                }

                if (Input.GetButton("RepairShip"))
                {
                    RepairShip();
                }
    ...
    void RepairShip()
    {
        int totalCurrency = int.Parse(currencyAmmount.gameObject.GetComponent<Text>().text);

        if (totalCurrency >= 50)
        {
            allStatus[healthLevel - 1].health += 50;
            Mathf.Clamp(allStatus[healthLevel - 1].health, 0, healthSlider.maxValue);
            totalCurrency -= 50;
            currencyAmmount.gameObject.GetComponent<Text>().text = totalCurrency.ToString();
            healthSlider.value = allStatus[healthLevel - 1].health;
        }
    ...
    void SetSliders()
    {
        specialCooldownSlider = specialCooldownBar.gameObject.GetComponent<Slider>();
        manaSlider = manaBar.gameObject.GetComponent<Slider>();
        slowMotionSlider = slowMotionBar.GetComponent<Slider>();
    ...
    void SlowMotionEffect()
    {
        ...
        slowMotionSlider.value = currentslowMotionTime;
